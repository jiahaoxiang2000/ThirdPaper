...
%% one point is create the full parallelism of the SPHINCS+ algorithm, not only on the SPHINCS+, also on it used hash functions, to achieve the best performance on the GPU.

%% consider the effect of the parallelism on the GPU, let the one thread do the most work. not just only consider the maximum throughput and minimum latency.
