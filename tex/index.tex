...
This implementation decomposes hash functions into parallel tasks rather than treating them as atomic operations, reducing cryptographic latency through fine-grained parallelization.

\subsubsection{WOTS\texorpdfstring{$^+$}{+} Parallelization}

Concurrent computation of $l$ independent hash chains by assigning chains or segments to distinct threads, when all the chains computer finish, then need to combined to one left node, if let the result to global memory, it will be slower, so here we storage the inter value to shared memory on GPU. For hypertree structures, multiple Merkle tree leaf root is independent, so the different Merkle tree WOTS\texorpdfstring{$^+$}{+} public keys are generated simultaneously, with shared memory optimizing authentication path construction. those different with the \cite{Kim2024}.

\subsubsection{FORS Parallelization}

Parallel generation of $k \times 2^a$ secret key elements and leaf nodes, followed by concurrent construction of $k$ Merkle trees. Shared memory aggregates tree roots to minimize global memory accesses during public key derivation.

\subsubsection{Hypertree Parallelization}
Concurrent construction of multiple Merkle trees across all $d$ layers. Threads compute nodes at identical levels but in different trees, enabling simultaneous execution of WOTS$^+$ key generation, hash chain computation, and node hashing operations.
